...
% @Nicolas/Edouard: faites ici la liste des sections qui sont pretes a etre relues.
% Section 1: il manque les contributions
% Section 2: OK. On pense éventuellement rajouter une hypothèse de contractance de Tau (pas sûr que ce soit le cas)
% Section 3: OK
% Section 4.1: OK dans un premier temps, mais on va peut-être modifier. En fait, on remplace pi_greedy(Q) par pi_hull(Q), et on ne peut faire cela que lorsque Q est concave. Deux possibilité: soit on dit juste que Tau_greedy est remplacé par Tau_hull, qui admet également Q* comme point fixe (Tau_greedy(Q*) = Tau_hull(Q*) puisque Q* concave d'après le lemme). Soit on est intéressé par ce qui se passe le long de la trajectoire Q_k (par exemple si on suppose Tau_greedy contractant), et pour conserver cette propriété il faudrait alors montrer que Tau conserve la concavité et que donc on a bien Tau_hull = Tau_greedy tout le long de la trajectoire. Il faut aussi reprendre les notations des graphiques.
% Section 4.2 et 4.3: KO. Pas encore retravaillé, mais ça ne devrait pas trop bougé
% Section 5: OK pour environnements et results, KO pour discussion
% Section 6: KO
% Section 7: KO
...
%     V^\pi(\os) &= \expectedvalue\left[ G^\pi \condbar \ov{s_0} = \os\right] \\
%     Q^\pi(\os, \oa) &= \expectedvalue\left[ G^\pi \condbar \ov{s_0} = \os, \ov{a_0} = \oa\right]
% \end{align}
\begin{equation}
    \label{eq:value-function}
...
\begin{assumption}[Existence of optimal policy]
...
\begin{remark}[Independence on the representative]
...
$Q_{0} \leftarrow 0$\;
\Repeat{convergence}{
    $Q_{k+1} \leftarrow \argmin_\theta \cL(Q_\theta, \cT Q_k; \cD)$\;
}
...
As stated in Remark~\ref{rmk:greedy}, computing the greedy policy $\pi_\text{greedy}$ in \eqref{eq:bellman-optimality} is not trivial since it requires solving the nested non-linear constrained optimisation program \eqref{eq:pi_greedy}. However, it can be solved efficiently by exploiting the \emph{structure} of the optimal value function $V^*$. % In the following, we say that a vector-valued function is concave and non-decreasing with respect to a variable if all of its components are.

%\begin{lemma}[Concavity and monotony of $Q^*$]
%\label{lemma:concavity}
%$Q^*$ is non-decreasing concave with respect to $\beta_a$. 
%\end{lemma}
%\begin{lemma}[Conservation of concavity and monotony by $\cT$]
%\label{lemma:tau_concavity}
%For any $Q\in(\Real^2)^{\ocS\ocA}$ non-decreasing concave with respect to $\beta_a$, $\cT Q$ is also non-decreasing concave with respect to $\beta_a$.
%\end{lemma}

%These two Lemmas can be used to derive an efficient computation method for $\pi_\text{greedy}$.

%\begin{lemma}[Hull policy equality]
%\label{lemma:pi_hull}
%For any $Q\in(\Real^2)^{\ocS\ocA}$ non-decreasing concave with respect to $\beta_a$, $\pi_\text{greedy}$ in \eqref{eq:pi_greedy} can be computed by running Algorithm~\ref{algo:pi_hull}.
%\begin{equation*}
...
\lIf{$q_r^0 = q_r^1$}{
\Return $\delta(a^0, \beta_a^0)$
}\Else{
$p \leftarrow (\beta - q_c^0) / (q_c^1 - q_c^0)$\;
\Return the mixture $(1-p)\delta(a^0, \beta_a^0) + p\delta(a^1, \beta_a^1)$\;
}
